<!DOCTYPE html>
<html lang="nl">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="Welkom bij onze menu pagina Indian Chillies">
    <meta name="author" content="Kishan">
    <meta name="keywords" content="Indian Chillies Restaurant, Indian cuisine, spicy food, Indian restaurant, dining experience, menu, food, chicken, chicken curry, vindaloo, samosa">
    <title>Menu pagina</title>
    <link rel="stylesheet" type="text/css" href="Css/styl.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Open+Sans:ital,wght@0,300..800;1,300..800&display=swap" rel="stylesheet">  
    <script src="lib/imageresize.js" defer></script>
    <script src="lib/hamburger.js" defer></script> 
</head>
<body>
    <?php include 'header.php'; ?>
    <main>
        <section class="containerr">
            <h1>Menu</h1>
        <article class="image_container">
            <img class="image" src="Images/Menuv1.png" alt="">
            <img class="image" src="Images/menuv2.png" alt="">
        </article>
        <article class="popup-image">
            <span>&times;</span>
            <img src="Images/Menuv1.png" alt="">

        </article>
    </section>
    </main>
    <?php include 'footer.php'; ?>
</body>
</html>
